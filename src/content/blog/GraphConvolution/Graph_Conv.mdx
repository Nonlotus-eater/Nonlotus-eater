...
$$
\theta_{*g}(x) &= U \theta(\Lambda) U^T x \\
            &= U [\Sigma_{k=0}^\infty \theta_k T_k(\tilde{\Lambda})] U^T x \\
            &= \Sigma_{k=0}^\infty \theta_k [U T_k(\tilde{\Lambda}) U^T x] \\ 
            &= \Sigma_{k=0}^\infty \theta_k [T_k(U \tilde{\Lambda} U^T)] x 
$$

带入式一，可得：

$$
原式 &= \Sigma_{k=0}^\infty \theta_k [T_k(2 \cdot (\frac{L}{\lambda_{max}} - I_n))] x \\
    &= \Sigma_{k=0}^\infty \theta_k T_k(\tilde{L}) x
$$
